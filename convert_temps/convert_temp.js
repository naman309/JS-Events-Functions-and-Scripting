// Using Strict Mode in JavaScript
"use strict";

var $ = function(id) { 
    return document.getElementById(id);
 };

var clearTextBoxes = function() {
    $("degrees_entered").value = "";
    $("degrees_computed").value = "";
};

function convertTemp(){
    
    var input_Degree = $("degrees_entered").value;
    
    if (isNaN(input_Degree) || input_Degree == ""){
        alert("You must enter a valid number for degrees.");
    }
    
    else if ($("to_celsius").checked){
        var celsiusTemp = (input_Degree - 32) * 5 / 9;
        $("degrees_computed").value = celsiusTemp;
    
    }else if ($("to_fahrenheit").checked){
        var fahrenheitTemp = (9 * input_Degree / 5) + 32;
        $("degrees_computed").value = fahrenheitTemp;
    }
    $("degrees_entered").focus();
}

// Creating celcius and fahrenheit functions
function toCelsius() {
    $("degree_label_1").innerHTML = "Enter F degrees:";
    $("degree_label_2").innerHTML = "Degrees Celsius:";
    clearTextBoxes();
}

function toFahrenheit() {
    $("degree_label_1").innerHTML = "Enter C degrees:";
    $("degree_label_2").innerHTML = "Degrees Fahrenheit:";
    clearTextBoxes();
}

// Using OnLoad and focus
window.onload = function() {
    $("convert").onclick = convertTemp;
    $("to_celsius").onclick = toCelsius;
    $("to_fahrenheit").onclick = toFahrenheit;
	$("degrees_entered").focus();
};